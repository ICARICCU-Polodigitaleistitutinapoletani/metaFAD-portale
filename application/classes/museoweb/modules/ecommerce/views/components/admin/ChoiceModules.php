<?php
class museoweb_modules_ecommerce_views_components_admin_ChoiceModules extends pinax_components_HtmlFormElement
{
    function render($outputMode=NULL, $skipChilds=false)
    {
        ...
        foreach( $modules as $k=>$m) {
            ...
            $output .=
                    '<tr>'.
                    '<td>'.$m->name.'</td>'.
                    '<td>'.$checkbox.pinax_helpers_Html::hidden($moduleId, '').'</td>'.
...
$(function(){
            ...
            $.ajax({
                    ...
                    success: function(data) {
                        if (data.status===true) {
                            ...
                            $('#s1_'+moduleId).prop('disabled', false).html(Pinax.template.render('ecomm-options', {items: data.result.fields, value: value.idField || ''}));
                            $('#s2_'+moduleId).prop('disabled', false).html(Pinax.template.render('ecomm-options', {items: data.result.media, value: mediaField}));
                            $('#s3_'+moduleId).prop('disabled', false).html(Pinax.template.render('ecomm-options', {items: _.union(data.result.fields, data.result.media), value: checkField}));
                        }
                    }
            });
        } else {
    ...
    Pinax.events.on("pinaxcms.formEdit.onReady", function(){
        $('input.js-ecomm').each(function(index, el){
            var \$el = \$(el);
            var \$elHidden = \$el.next();
            var value = \$elHidden.val();
    ...
    Pinax.template.define('ecomm-options', ''+
        '<option value=""></option>'+
