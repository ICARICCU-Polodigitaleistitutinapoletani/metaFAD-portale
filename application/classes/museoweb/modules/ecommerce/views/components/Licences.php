<?php
class museoweb_modules_ecommerce_views_components_Licences extends pinax_components_RecordSetList
{
    function init()
    {
    ...
    function process()
    {
        ...
        if ($modulePref) {
            ...
            if ($ar->{$modulePref->checkField->id}) {
                $iterator = pinax_ObjectFactory::createModelIterator('museoweb.modules.ecommerce.models.Licence', 'licencesToBuy');

                $this->_content->records = new pinax_components_RecordSetListIterator( $this->_application, $this, $iterator, $this->routeUrl, $cssClass, $this->getAttribute('processCell'), $ar->getId() );
