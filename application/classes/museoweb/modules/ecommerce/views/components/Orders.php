<?php
class museoweb_modules_ecommerce_views_components_Orders extends pinax_components_Component
{
    function process()
    {
        ...
        foreach($it as $ar) {
            if ( $orderId != $ar->order_id ) {
                if ( count( $items ) ) {
                    pinax_helpers_Array::arrayMultisortByLabel( $items, 'orderitem_title' );
                    $this->_content[ 'records' ][] = array( 'date' => $orderDate2, 'items' => $items );
                }
                $items = array();
                $orderId = $ar->order_id;
                $orderDate2 = pinax_defaultDate2locale( __T( PNX_DATETIME_FORMAT ), $ar->order_date );
            }
        ...
        if ( count( $items ) ) {
            pinax_helpers_Array::arrayMultisortByLabel( $items, 'title' );
            $this->_content[ 'records' ][] = array( 'date' => $orderDate2, 'items' => $items );
        }

        $url =  __Session::get('museoweb.ecommerce.download');
        if ($url) {
            $this->_content[ 'redirect' ] = $url;
            __Session::remove('museoweb.ecommerce.download' );
        }
    }
}
