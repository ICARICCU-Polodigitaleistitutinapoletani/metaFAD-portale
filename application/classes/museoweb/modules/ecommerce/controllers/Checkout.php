<?php
class museoweb_modules_ecommerce_controllers_Checkout extends pinax_mvc_core_Command
{
    public function execute()
    {
        ...
        $arOrder->order_date = new pinax_types_DateTime();
        ...
        $arItem = pinax_ObjectFactory::createModel( 'museoweb.modules.ecommerce.models.OrderItem' );
        $total = 0;
        $items = array();
        foreach($cart as $k=>$v) {
        ...
        if ($gateway) {
            if (!$gateway->pay($this->user, $orderId, $orderCode, $total, $items)) {
                pinax_application_MessageStack::add(__T('Errore nella procedura di pagamento'), PNX_MESSAGE_ERROR);
