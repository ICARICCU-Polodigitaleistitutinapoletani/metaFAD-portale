...
class iiif_services_Manifest extends PinaxObject
{
    ...
    public function getManifest($uid, $data, $type, $page = null)
    {
        ...
        if($page) {
            $sequence->startCanvas = PNX_HOST . '/metadata/' . $page . '/canvas/' . $page . '.json';
        }
    ...
    public function getCanvas($image)
    {
        ...
        $canvas = new StdClass();
        $canvas->{'@id'} = PNX_HOST . '/metadata/' . $image->id . '/canvas/' . $image->id . '.json';
        $canvas->{'@type'} = 'sc:Canvas';
        $canvas->width = $size[0];
        $canvas->height = $size[1];
        $canvas->images[] = $this->getAnnotation($image);
        $thumbnail = new stdClass();
        $thumbnail->{"@id"} = PNX_HOST . '/iiif/' . $uuidThumbnail . '/full/full/0/default.jpg';
    ...
    {
        $ar = __ObjectFactory::createModel('mibac.1418.iiif.models.Document');
    ...
    {
        $uri = PNX_HOST . '/metadata/' . $image->id . '/annotation/' . $image->id . '.json';
    ...
    public function getResource($image)
    {
        ...
        $resource->{'@id'} = PNX_HOST . '/iiif/' . $uuid . '/full/full/0/default.jpg';
        ...
        $resource->service->{"@id"} = PNX_HOST . '/iiif/' . $uuid;
    ...
    private function getInfos()
    {
        ...
        foreach ($intervals as $interval) {
            $url = __Config::get('metafad.solr.dam.url');
            if(implode(' OR ', array_slice($this->ids, $interval[0] - 1, 50)) != '')
            {
                ...
                $request = pinax_ObjectFactory::createObject('pinax.rest.core.RestRequest', $url . '/select?', 'POST', http_build_query($postBody));
    ...
    private function getDescription($metadata, $uid, $type)
    {
        ...
        $description .= '<br/><a href="' . __Link::makeUrl('opac_detail', ['id' => $uid]) . '" target="_blank">Vedi scheda</a>';
        return $description;
    }
        ...
        if ($ls->children) {
                ...
                foreach ($childrenStructures as $cs) {
                    foreach ($data->physicalSTRU->image as $image) {
                        if (str_replace('range-', '', $cs->{'@id'}) == $image->keyNode) {
                            $cs->canvases[] = PNX_HOST . '/metadata/' . $image->id . '/canvas/' . $image->id . '.json';;
                        }
                    }
                }
            }
            ...
            foreach ($data->physicalSTRU->image as $image) {
                if ($ls->key == $image->keyNode) {
                    $range->canvases[] = PNX_HOST . '/metadata/' . $image->id . '/canvas/' . $image->id . '.json';;
                }
            }
        }
