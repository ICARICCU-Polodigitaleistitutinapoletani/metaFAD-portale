...
{
    const ADD_BREADCRUMB = 'addBreadcrumb';
    ...
    static function documentUIDwithSearch($uid, $pos)
    {
        $searchParams = '';
        $filtersCode = pinax_ObjectValues::get('mibac', 'savedSearch', __Request::get('search', ''));
        $searchPos = __Request::get('searchPos', '');
        if ($filtersCode) {
            $savedSearch = __Session::get('mibac.savedSearch', array());
            if (isset($savedSearch[$filtersCode])) {
                $recordPos = $searchPos ? $searchPos : $savedSearch[$filtersCode]['start'] + $pos;
                $searchParams = '?search=' . $filtersCode . '&searchPos=' . $recordPos;
            }
        }
        return $uid . $searchParams;
    }
